...
def analyze_stock(ticker, days):
    ...
    if pd.notna(latest['RSI']) and latest['RSI'] < 30:
        buy_signals += 2
        score += 3
    if pd.notna(latest['Stoch']) and latest['Stoch'] < 20:
        buy_signals += 2
        score += 2
    if pd.notna(latest['BB_lower']) and latest['Close'] < latest['BB_lower']:
        buy_signals += 1
        score += 2
    if pd.notna(latest['SMA20']) and pd.notna(latest['SMA50']) and latest['Close'] > latest['SMA20'] > latest['SMA50']:
        buy_signals += 1
        score += 2
    ...
    if pd.notna(latest['RSI']) and latest['RSI'] > 70:
        sell_signals += 2
        score -= 3
    if pd.notna(latest['Stoch']) and latest['Stoch'] > 80:
        sell_signals += 2
        score -= 2
    ...
    # Require multiple signals and check for conflicts
    if buy_signals >= 3 and sell_signals == 0:
        recommendation['action'] = 'BUY'
        recommendation['when'] = f"{(today + timedelta(days=1)).strftime('%Y-%m-%d')} at 9:15 AM IST"
        recommendation['reason'] = f"Strong buy signals (RSI: {latest['RSI']:.2f}, Stoch: {latest['Stoch']:.2f}, MACD: {latest['MACD']:.4f})"
    elif sell_signals >= 3 and buy_signals == 0:
        recommendation['action'] = 'SELL'
        recommendation['when'] = f"{(today + timedelta(days=1)).strftime('%Y-%m-%d')} at 9:15 AM IST"
        recommendation['reason'] = f"Strong sell signals (RSI: {latest['RSI']:.2f}, Stoch: {latest['Stoch']:.2f}, MACD: {latest['MACD']:.4f})"
    else:
        recommendation['action'] = 'HOLD'
        recommendation['when'] = f"{today.strftime('%Y-%m-%d')} at any time"
        reason = "Mixed or weak signals"
        if buy_signals > 0 and sell_signals > 0:
            reason = f"Conflicting signals (Buy: {buy_signals}, Sell: {sell_signals})"
        elif buy_signals > 0:
            reason = f"Insufficient buy signals ({buy_signals}/3)"
        elif sell_signals > 0:
            reason = f"Insufficient sell signals ({sell_signals}/3)"
    ...
    # Trend prediction
    trend = 'Uncertain'
    if buy_signals >= 3 and pd.notna(latest['ADX']) and latest['ADX'] > 20:
        trend = 'UP'
    elif sell_signals >= 3 and pd.notna(latest['ADX']) and latest['ADX'] > 20:
        trend = 'DOWN'
...
def predict_best_sell_day(df, buy_date):
    """Predict the best day to sell after buying, including estimated sell price"""
    last_5 = df.tail(5)
    latest = df.iloc[-1]
    ...
    buy_datetime = datetime.strptime(buy_date.split(" at")[0], '%Y-%m-%d')
    days_from_today = (buy_datetime.date() - datetime.now().date()).days
    
    days_ahead = 1
    for i in range(1, 8):
        future_date = buy_datetime + timedelta(days=i)
        est_rsi = latest['RSI'] + rsi_trend * (i + days_from_today) if pd.notna(latest['RSI']) else 50
        est_price = latest['Close'] + price_trend * (i + days_from_today)
        est_macd = latest['MACD'] + macd_trend * (i + days_from_today) if pd.notna(latest['MACD']) else 0
        
        if (est_rsi > 70) or \
           (pd.notna(latest['BB_upper']) and est_price > latest['BB_upper']) or \
           (pd.notna(latest['SMA20']) and est_price > latest['SMA20'] and est_macd < 0):
            return {
                'when': f"{future_date.strftime('%Y-%m-%d')} at 9:15 AM IST",
                'price': est_price,
                'reason': 'Predicted overbought or bearish crossover'
            }
        days_ahead = i + 1
    
    est_price_default = latest['Close'] + price_trend * (7 + days_from_today)
    return {
        'when': f"{(buy_datetime + timedelta(days=7)).strftime('%Y-%m-%d')} at 9:15 AM IST",
        'price': est_price_default,
        'reason': 'No strong sell signal in next 7 days; consider holding or monitoring'
    }
...
search_ticker = st.sidebar.text_input("Search for a Stock (e.g., TATAMOTORS.NS)", "").strip().upper()
# Auto-append .NS if missing
if search_ticker and not search_ticker.endswith('.NS'):
    search_ticker = f"{search_ticker}.NS"
    st.sidebar.info(f"Appended .NS to search: {search_ticker}")
        ...
        for ticker in tickers:
            with st.spinner(f"Analyzing {ticker}..."):
                ...
                if result:
                    ...
                    st.error(f"Failed to analyze {ticker}. Data unavailable or insufficient.")
    ...
    if search_ticker:
        with st.spinner(f"Analyzing {search_ticker}..."):
            ...
            if search_result:
                ...
                # Add to summary if not already in selected tickers
                if search_ticker not in tickers:
                    results[search_ticker] = search_result
                    summary_data.append(create_summary_row(search_ticker, search_result))
                    if search_result['score'] > best_score:
                        best_score = search_result['score']
                        best_stock = search_ticker
            else:
                st.error(f"Could not analyze {search_ticker}. Please check the ticker (e.g., ensure it ends with .NS) or try again later.")
    
    # Display summary and best stock
    if results:
        st.subheader("📊 Summary of Analyzed Stocks")
        ...
        if best_stock:
            ...
            buy_price = results[best_stock]['latest']['Close']
            st.write(f"**Buy Recommendation:** {results[best_stock]['recommendation']['action']} on {results[best_stock]['recommendation']['when']} at ₹{buy_price:.2f}")
            buy_date = results[best_stock]['recommendation']['when']
            sell_prediction = predict_best_sell_day(results[best_stock]['df'], buy_date)
            st.write(f"**Sell Prediction:** Sell on {sell_prediction['when']} at ₹{sell_prediction['price']:.2f} - {sell_prediction['reason']}")
            profit = sell_prediction['price'] - buy_price
            profit_percent = (profit / buy_price) * 100
            profit_color = "green" if profit > 0 else "red"
            st.write(f"**Estimated Profit/Loss:** <span style='color:{profit_color}'>₹{profit:.2f} ({profit_percent:.2f}%)</span>", unsafe_allow_html=True)
    else:
        st.error("No stocks could be analyzed. Please select stocks or enter a valid ticker.")

# Instructions
st.sidebar.info("Select stocks to compare and/or search for a stock (e.g., TATAMOTORS.NS). Adjust the period and click 'Run Analysis'.")